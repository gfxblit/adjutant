...
def mock_fs():
    ...
    # We don't use mock_open() here because we want to handle multiple files specifically
...
@patch("adjutant.engine.timezone")
@patch("adjutant.engine.datetime")
def test_spawn_agent_comprehensive(mock_datetime, mock_timezone, mock_get_root, mock_fs, mock_subp):
    # Setup
    ...
    mock_datetime.now.return_value.isoformat.return_value = "2026-03-24T12:00:00+00:00"
    ...
    assert redirect_path in files
    files[redirect_path].write.assert_called_once_with(os.path.join(project_root, ".beads"))
    ...
    assert "SCV SPAWN: 2026-03-24T12:00:00+00:00" in all_writes
    assert f"AGENT: {agent_name}" in all_writes
    assert CODER_PROMPT in all_writes
    ...
    assert scv_info_path in files
    info_handle = files[scv_info_path]
...
@patch("adjutant.engine.timezone")
@patch("adjutant.engine.datetime")
def test_spawn_agent_custom_model_and_directive(mock_datetime, mock_timezone, mock_get_root, mock_fs, mock_subp):
    # Setup
    ...
    def open_side_effect(path, mode='r', **kwargs):
        if path not in files:
            ...
            files[path] = m
        m = files[path]
        if 'r' in mode and "system.md" in path:
    ...
    # Verify model and directive used in Popen
    args, kwargs = mock_subp["popen"].call_args
    cmd = args[0]
    assert "--model" in cmd
    ...
    assert custom_directive in all_writes
