...
def test_spawn_agent_scv_coder(
    mock_exists, mock_makedirs, mock_popen, mock_run, mock_get_root
):
    # Setup
...
@patch("os.makedirs")
@patch("os.path.exists")
def test_spawn_agent_scv_tester(
    mock_exists, mock_makedirs, mock_popen, mock_run, mock_get_root
):
    # Setup
...
@patch("os.makedirs")
@patch("os.path.exists")
def test_spawn_agent_logs_prompt_and_command(
    mock_exists, mock_makedirs, mock_popen, mock_run, mock_get_root
):
    # Setup
    agent_name = "scv-coder"
...
def test_spawn_agent_invalid_name():
    with pytest.raises(
        ValueError, match="Unknown agent or missing system prompt: invalid-agent"
    ):
