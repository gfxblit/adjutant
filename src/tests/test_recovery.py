...
class TestRecovery(unittest.TestCase):
    ...
    def test_recover_orphaned_scvs(
        self, mock_cleanup, mock_is_running, mock_listdir, mock_exists, mock_isdir
    ):
        # Setup mocks
    ...
    def test_cleanup_scv(self, mock_logger, mock_remove, mock_exists, mock_run):
        ...
        worktree_path = os.path.join(
            project_root, ".adjutant", "worktrees", objective_id
        )
        resolved_path = os.path.join(
            project_root,
            ".adjutant",
            "worktrees",
            f".resolved_system_{objective_id}.md",
        )
    ...
    @patch("os.path.exists")
    @patch("os.remove")
    def test_cleanup_scv_push_failure(
        self, mock_remove, mock_exists, mock_run, mock_logger
    ):
        project_root = "/tmp/project"
        objective_id = "test-obj"
        worktree_path = os.path.join(
        ...
        mock_logger.error.assert_any_call(
            "An unexpected error occurred during git push for scv/test-obj: Push failed"
        )
    ...
    @patch("os.path.exists")
    @patch("os.remove")
    def test_cleanup_scv_commit_failure(
        self, mock_remove, mock_exists, mock_run, mock_logger
    ):
        project_root = "/tmp/project"
        objective_id = "test-obj"
        ...
        # Verify logger was called for the failure
        mock_logger.error.assert_any_call(
            "An unexpected error occurred during auto-commit for /tmp/project/.adjutant/worktrees/test-obj: Commit failed"
        )
    ...
    @patch("os.path.exists")
    @patch("os.remove")
    def test_cleanup_scv_resolved_path_remove_failure(
        self, mock_remove, mock_exists, mock_run, mock_logger
    ):
        project_root = "/tmp/project"
        objective_id = "test-obj"
        ...
        # Verify logger was called for the failure
        mock_logger.error.assert_any_call(
            "Failed to remove resolved system prompt: Failed to remove resolved path"
        )
