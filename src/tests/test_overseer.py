...
class TestSCVOverseer(unittest.TestCase):
    ...
    def test_overseer_restarts_on_quota_crash(
        self, mock_spawn, mock_open_file, mock_kill, mock_exists, mock_get_registry
    ):
        ...
        mock_spawn.assert_called_with(
            "scv-coder", "obj-123", starting_model="gemini-3-flash-preview"
        )
    ...
    @patch("builtins.open")
    @patch("adjutant.engine.spawn_agent")
    def test_overseer_handles_resource_exhausted(
        self, mock_spawn, mock_open_file, mock_kill, mock_exists, mock_get_registry
    ):
        # Mock registry from worktrees
        registry_data = {
...
class TestSCVOverseerWorktrees(unittest.TestCase):
    ...
    def test_get_registry_from_worktrees(
        self, mock_open_file, mock_isdir, mock_listdir, mock_exists
    ):
        # Setup
    ...
    def test_check_scvs_crashed_restart(
        self,
        mock_cleanup,
        mock_spawn,
        mock_open_file,
        ...
        # Verify restart
        mock_spawn.assert_called_with(
            "scv-coder", "obj-crash", starting_model="gemini-3-flash-preview"
        )
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("adjutant.engine.cleanup_scv")
    def test_check_scvs_crashed_cleanup(
        self, mock_cleanup, mock_open_file, mock_kill, mock_exists, mock_get_registry
    ):
        # Setup registry
        registry = {
            "obj-done": {
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("adjutant.engine.cleanup_scv")
    def test_check_scvs_exhausted_cleanup(
        self, mock_cleanup, mock_open_file, mock_kill, mock_exists, mock_get_registry
    ):
