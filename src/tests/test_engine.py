...
class TestAdjutantHUD(unittest.TestCase):
    ...
    def test_update_hud_success(
        self, mock_exists, mock_flush, mock_write, mock_check_output
    ):
        ...
        expected_title = (
            "\033]0;Mission: Test Mission | 70.0% | 7/10 | Open: 3, IP: 0\007"
        )
    ...
    @patch("sys.stdout.write")
    @patch("os.path.exists")
    def test_update_hud_handles_subprocess_error(
        self, mock_exists, mock_write, mock_check_output
    ):
        import subprocess
        ...
        expected_title = (
            "\033]0;Mission: Test Mission | 0.0% | 0/0 | Open: 0, IP: 0\007"
        )
        mock_write.assert_called_with(expected_title)
    ...
    def test_update_hud_edge_cases(self, mock_write, mock_check_output, mock_get_scvs):
        ...
        hud.update_hud()
        mock_write.assert_called_with(
            "\033]0;Mission: Test Mission | 100.0% | 5/5 | Open: 0, IP: 0\007"
        )
...
class TestRunAdjutantAgent(unittest.TestCase):
    ...
    def test_run_adjutant_agent_writes_resolved_prompt(
        self,
        mock_setup_logging,
        mock_recover,
        mock_overseer_class,
    ...
    @patch("adjutant.engine.recover_orphaned_scvs")
    @patch("adjutant.engine.setup_logging")
    def test_run_adjutant_agent_hud_integration(
        self,
        mock_setup_logging,
        mock_recover,
        mock_overseer_class,
    ...
    def test_run_adjutant_agent_gemini_not_found(
        self,
        mock_logger,
        mock_setup_logging,
        mock_recover,
