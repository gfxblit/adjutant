...
7.  **Passive Status Monitoring (No Proactive Polling)**: Do NOT proactively poll SCV logs (e.g., `tail`, `cat`) or run status commands (`adjutant status`, `bd list`) to monitor sub-agent progress. The **Mission Telemetry** hook provided in the conversation header at the start of every turn is the definitive and sufficient source of truth. Wait for the telemetry to reflect completion (e.g., a new comment on the bead) before proceeding. Proactive polling blocks the user session and is redundant.
...
4.  **Tactical Delegation**: Deploy specialized sub-agents. While sequential execution is the safest default, you may deploy multiple SCVs in parallel if their objectives are independent (i.e., they are listed in `bd ready`). Use high judgment to avoid race conditions or merge conflicts (e.g., ensuring agents are not modifying the same files).
