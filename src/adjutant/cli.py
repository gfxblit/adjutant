...
def main():
    ...
    plan_parser = subparsers.add_parser(
        "plan",
        help="Mission planning (default). Accepts an optional mission directive.",
    )
    ...
    run_agent_parser.add_argument(
        "--directive", help="Custom mission directive for the agent"
    )

    # logs subcommand
    logs_parser = subparsers.add_parser("logs", help="Show logs for a mission or SCV")
    logs_parser.add_argument(
        "objective_id", nargs="?", help="Objective ID (e.g. h6z or adjutant-h6z)"
    )
    logs_parser.add_argument(
    ...
    subparsers.add_parser(
        "recover", help="Recover stranded SCV work from orphaned worktrees"
    )
